...
public class RESTPrompterTest extends AbstractPrompterTest
{
    ...
    public void advancedPropertiesAreValid() throws PrompterException
    {
        ...
        when(prompter.prompt("Add Dispatcher?", PluginModulePrompter.YN_ANSWERS, "N")).thenReturn("y")
                .thenReturn("N");
        ...
        assertEquals("wrong number of packages", 1, props.getPackagesToScan()
                .size());
        assertEquals("wrong package", packageToScan, props.getPackagesToScan()
                .get(0));

        assertEquals("wrong number of dispatcher", 1, props.getDispatchers()
                .size());
        assertEquals("wrong dispatcher", "FORWARD", props.getDispatchers()
                .get(0));
    }
}
